...
@property (nonatomic, strong) TVUPLSection *clickSection;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    TVUPLSection *findSection = nil;
    TVUPLRow *findRow = nil;
    for (TVUPLRow *row in self.section.rows) {
        if (row.hidden == NO && CGRectContainsPoint(row.bindView.frame, point)) {
            findRow = row;
            break;
        }
    }
    ...
    self.clickSection = findSection;
    self.clickRow = findRow;
    
    findRow.backView.backgroundColor = [[UIColor lightGrayColor] colorWithAlphaComponent:0.1];
}
